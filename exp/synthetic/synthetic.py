...
def main(args, importance_func, impant_model, interpret_net, dataset):
    if args.cuda:
        interpret_net.cuda()
        impant_model.cuda()

    # note: the load helper is subsumed by making dataset an argument to main()
    # Load data
    ...
    for img_idx in range(args.image_offset, args.image_offset + args.num_imgs):
        ...
        img_idx = rng.choice(range(args.num_examples))
        img_loader, img_filename, gt_class_name, pred_class_name, classifier_output = \
            utils.get_imgnet_one_image_loader(
                    dataset, interpret_net, 
                    img_idx, args.batch_size, 
                    target_label='gt', cuda=args.cuda)
        dirname = '%s/%s' % (args.save_dir, args.gen_model_name)
        if not os.path.exists(dirname):
            os.makedirs(dirname)
        output_file = '%s/%s_records.th' % (dirname, img_filename)
        ...
        unnormalized_img = orig_img.squeeze(0)
        unnormalized_img = unnormalized_img.repeat(3, 1, 1)
...
def parse_args():
    ...
    parser.add_argument('--classifier', type=str, default='neuralnet',
                        help='Choose from [neuralnet, logisticregression]')
    parser.add_argument('--window', type=int, default=1)
    parser.add_argument('--num-samples', type=int, default=1)
    parser.add_argument('--num-examples', type=int, default=1)  # number of training data
