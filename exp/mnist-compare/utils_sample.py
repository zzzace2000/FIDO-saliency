...
class cond_sampler_imagenet:
        ...
        if os.path.exists(path_mean + '.npy') and os.path.exists(path_cov + '.npy'):
            ...
            for c in [0, 1, 2]:
                ...
                for i in xrange(int(self.num_samples_fit / X.shape[0]) + 1):
                    ...
                    idx = \
                    random.sample(range((self.image_dims[0] - self.patchSize) * (self.image_dims[1] - self.patchSize)),
                                  1)[0]
                    idx = np.unravel_index(idx,
                                           (self.image_dims[0] - self.patchSize, self.image_dims[1] - self.patchSize))
                    idx = [idx[0], idx[1]]
    ...
    def _get_cond_params(self, surrPatch, inPatchIdx, channel):
        ...
        path_dotProdForMean = self.path_folder + '{}_cov{}_win{}_dotProdForMean_{}_{}'.format(self.netname,
                                                                                              self.patchSize,
                                                                                              self.win_size,
                                                                                              inPatchIdx[0],
                                                                                              inPatchIdx[-1])
        ...
        if not os.path.exists(path_dotProdForMean + '.npy'):
            ...
            cov12 = np.delete(self.covMats[channel][inPatchIdx, :], inPatchIdx, axis=1) if np.ndim(
                inPatchIdx > 1) else np.delete(self.covMats[channel][inPatchIdx, :], inPatchIdx)
        ...
        path_condCov = self.path_folder + '{}_cov{}_win{}_cond_cov_{}_{}_indep'.format(self.netname, self.patchSize,
        ...
        if not os.path.exists(path_condCov + '.npy'):
            cov11 = self.covMats[channel][inPatchIdx][:, inPatchIdx]
            cov12 = np.delete(self.covMats[channel][inPatchIdx, :], inPatchIdx, axis=1) if np.ndim(
                inPatchIdx > 1) else np.delete(self.covMat[inPatchIdx, :], inPatchIdx)
            cov21 = np.delete(self.covMats[channel][:, inPatchIdx], inPatchIdx, axis=0)
            cov22 = np.delete(np.delete(self.covMats[channel], inPatchIdx, axis=0), inPatchIdx, axis=1)
    ...
    def _get_surr_patch(self, x, sampleIndices):
        ...
        inPatchIdx = np.tile(np.array([i for i in xrange(self.patchSize * self.patchSize)]), 3).reshape(3,
                                                                                                        self.patchSize,
                                                                                                        self.patchSize)
